// 첫 실행 때 DB 를 채운다.
//
//   node prisma/seed.mjs
//
// 이미 들어 있는 것은 건드리지 않는다. 배포 때마다 돌아도 안전하다.
// 관리자 비밀번호는 ADMIN_PASSWORD 로 바꿔 넣을 수 있다.

import { randomBytes, scryptSync } from 'node:crypto'
import { PrismaPg } from '@prisma/adapter-pg'
import { PrismaClient } from '@prisma/client'
import 'dotenv/config'

const db = new PrismaClient({
  adapter: new PrismaPg({ connectionString: process.env.DATABASE_URL }),
})

function hashPassword(password) {
  const salt = randomBytes(16).toString('hex')
  const key = scryptSync(password, salt, 64).toString('hex')
  return `scrypt:${salt}:${key}`
}

const digits = (phone) => phone.replace(/[^0-9]/g, '')

const USERS = [
  {
    id: 'u-admin',
    nickname: '운영자',
    hue: 265,
    phone: '010-0000-0000',
    verified: true,
    role: 'ADMIN',
    region: '서울 강남구',
    intro: '한판 운영팀입니다.',
    createdAt: '2026-07-01T09:00:00.000Z',
  },
  {
    id: 'u-midking',
    nickname: '미드장인',
    hue: 282,
    phone: '010-1234-5678',
    verified: true,
    region: '서울 강남구',
    intro: '10년째 롤만 합니다. 라인전 위주로 알려드려요.',
    photoUrl: '/mates/midking.jpg',
    photoStatus: 'APPROVED',
    createdAt: '2026-07-03T09:00:00.000Z',
    kept: 42,
    late: 1,
    ratingSum: 210,
    reviewCount: 43,
  },
  {
    id: 'u-newbie',
    nickname: '초보탈출',
    hue: 200,
    phone: '010-2222-3333',
    verified: true,
    region: '서울 마포구',
    intro: '이번에 발로란트 시작했어요. 기초부터 배우고 싶습니다!',
    photoUrl: '/mates/newbie.jpg',
    photoStatus: 'APPROVED',
    createdAt: '2026-07-20T09:00:00.000Z',
    kept: 3,
    late: 1,
    cancelLate: 1,
    ratingSum: 24,
    reviewCount: 5,
  },
  {
    id: 'u-nightowl',
    nickname: '한밤의궁수',
    hue: 320,
    phone: '010-4444-5555',
    verified: true,
    region: '경기 성남시 분당구',
    intro: '새벽에 같이 게임할 사람 구해요. 즐겜 지향입니다.',
    photoUrl: '/mates/nightowl.jpg',
    photoStatus: 'APPROVED',
    createdAt: '2026-06-11T09:00:00.000Z',
    kept: 18,
    late: 3,
    cancelLate: 2,
    noShow: 1,
    ratingSum: 96,
    reviewCount: 21,
  },
  {
    id: 'u-coach',
    nickname: '코치정',
    hue: 155,
    phone: '010-6666-7777',
    verified: true,
    region: '부산 부산진구',
    intro: 'FC온라인 전술 코칭. 감독모드 세팅 알려드립니다.',
    photoUrl: '/mates/coach.jpg',
    photoStatus: 'APPROVED',
    createdAt: '2026-05-02T09:00:00.000Z',
    kept: 67,
    cancelLate: 1,
    ratingSum: 336,
    reviewCount: 68,
  },
  {
    id: 'u-potato',
    nickname: '감자칩',
    hue: 30,
    phone: '010-8888-9999',
    verified: false,
    region: '서울 관악구',
    intro: '배그 스쿼드 한 자리 비어요. 마이크 필수!',
    photoUrl: '/mates/potato.jpg',
    photoStatus: 'APPROVED',
    createdAt: '2026-08-01T09:00:00.000Z',
  },
  {
    id: 'u-overtime',
    nickname: '야근전사',
    hue: 95,
    phone: '010-1010-2020',
    verified: true,
    region: '서울 광진구',
    intro: '퇴근하고 오버워치 같이 하실 분. 실력 상관없어요.',
    photoUrl: '/mates/overtime.jpg',
    photoStatus: 'APPROVED',
    createdAt: '2026-07-15T09:00:00.000Z',
    kept: 11,
    ratingSum: 55,
    reviewCount: 11,
  },
  {
    id: 'u-mapler',
    nickname: '단풍잎',
    hue: 15,
    phone: '010-3030-4040',
    verified: true,
    region: '경기 수원시 영통구',
    intro: '메이플 복귀 유저입니다. 요즘 뭐가 바뀌었는지 알려주실 분?',
    photoUrl: '/mates/mapler.jpg',
    photoStatus: 'APPROVED',
    createdAt: '2026-07-28T09:00:00.000Z',
    kept: 6,
    ratingSum: 29,
    reviewCount: 6,
  },
]

/**
 * 예시 회원들의 연결된 게임 계정.
 *
 * 실제 게임사에서 가져온 값이 아니라 화면을 보기 위한 예시다. 진짜 계정을
 * 연결하면 이 값들은 게임사가 주는 값으로 덮인다. 예시 회원 말고 실제
 * 회원이 한 명이라도 있으면 이 파일 전체가 건너뛰어진다.
 *
 * 한 사람이 여러 게임을 하는 모습도 보여야 해서 두세 개씩 붙였다.
 */
const RIOT_ACCOUNTS = [
  {
    id: 'ra-midking',
    userId: 'u-midking',
    puuid: 'seed-puuid-midking',
    gameName: '미드장인',
    tagLine: 'KR1',
    summonerId: 'seed-sid-midking',
    summonerLevel: 412,
    tier: 'MASTER',
    division: null,
    lp: 137,
    wins: 214,
    losses: 178,
    mainRole: 'MID',
    champions: [
      { id: 7, name: 'Leblanc', games: 6, wins: 4 },
      { id: 103, name: 'Ahri', games: 3, wins: 2 },
      { id: 245, name: 'Ekko', games: 1, wins: 1 },
    ],
    kills: 8.2,
    deaths: 4.1,
    assists: 6.9,
    recentGames: 10,
    verifiedAt: '2026-08-05T05:00:00.000Z',
    syncedAt: '2026-08-08T01:00:00.000Z',
  },
  {
    id: 'ra-nightowl',
    userId: 'u-nightowl',
    puuid: 'seed-puuid-nightowl',
    gameName: '한밤의궁수',
    tagLine: 'KR2',
    summonerId: 'seed-sid-nightowl',
    summonerLevel: 233,
    tier: 'PLATINUM',
    division: 'II',
    lp: 64,
    wins: 96,
    losses: 88,
    mainRole: 'ADC',
    champions: [
      { id: 22, name: 'Ashe', games: 5, wins: 3 },
      { id: 51, name: 'Caitlyn', games: 3, wins: 1 },
      { id: 222, name: 'Jinx', games: 2, wins: 1 },
    ],
    kills: 6.4,
    deaths: 5.2,
    assists: 7.7,
    recentGames: 10,
    verifiedAt: null,
    syncedAt: '2026-08-08T02:10:00.000Z',
  },
  {
    id: 'ra-newbie',
    userId: 'u-newbie',
    puuid: 'seed-puuid-newbie',
    gameName: '초보탈출',
    tagLine: 'KR3',
    summonerLevel: 41,
    tier: 'IRON',
    division: 'III',
    lp: 12,
    wins: 14,
    losses: 26,
    mainRole: 'SUPPORT',
    champions: [
      { id: 12, name: 'Alistar', games: 4, wins: 1 },
      { id: 412, name: 'Thresh', games: 2, wins: 1 },
    ],
    kills: 2.1,
    deaths: 7.8,
    assists: 9.3,
    recentGames: 8,
    verifiedAt: null,
    syncedAt: '2026-08-07T12:00:00.000Z',
  },
  {
    id: 'ra-coach',
    userId: 'u-coach',
    puuid: 'seed-puuid-coach',
    gameName: '코치정',
    tagLine: 'KR7',
    summonerId: 'seed-sid-coach',
    summonerLevel: 305,
    tier: 'DIAMOND',
    division: 'I',
    lp: 88,
    wins: 152,
    losses: 131,
    mainRole: 'TOP',
    champions: [
      { id: 86, name: 'Garen', games: 5, wins: 4 },
      { id: 122, name: 'Darius', games: 4, wins: 2 },
      { id: 24, name: 'Jax', games: 1, wins: 0 },
    ],
    kills: 5.6,
    deaths: 4.4,
    assists: 5.1,
    recentGames: 10,
    verifiedAt: '2026-08-06T05:00:00.000Z',
    syncedAt: '2026-08-08T03:20:00.000Z',
  },
  {
    id: 'ra-overtime',
    userId: 'u-overtime',
    puuid: 'seed-puuid-overtime',
    gameName: '야근전사',
    tagLine: 'KR9',
    summonerId: 'seed-sid-overtime',
    summonerLevel: 158,
    tier: 'GOLD',
    division: 'II',
    lp: 41,
    wins: 63,
    losses: 59,
    mainRole: 'JUNGLE',
    champions: [
      { id: 64, name: 'LeeSin', games: 6, wins: 3 },
      { id: 254, name: 'Vi', games: 3, wins: 2 },
    ],
    kills: 4.8,
    deaths: 5.9,
    assists: 8.2,
    recentGames: 9,
    verifiedAt: null,
    syncedAt: '2026-08-08T04:30:00.000Z',
  },
  {
    id: 'ra-potato',
    userId: 'u-potato',
    puuid: 'seed-puuid-potato',
    gameName: '감자칩',
    tagLine: 'KR5',
    summonerLevel: 92,
    tier: 'SILVER',
    division: 'IV',
    lp: 27,
    wins: 31,
    losses: 34,
    mainRole: 'SUPPORT',
    champions: [
      { id: 40, name: 'Janna', games: 4, wins: 2 },
      { id: 350, name: 'Yuumi', games: 3, wins: 1 },
    ],
    kills: 1.9,
    deaths: 4.6,
    assists: 12.4,
    recentGames: 8,
    verifiedAt: null,
    syncedAt: '2026-08-08T03:05:00.000Z',
  },
  {
    id: 'ra-mapler',
    userId: 'u-mapler',
    puuid: 'seed-puuid-mapler',
    gameName: '단풍잎',
    tagLine: 'KR4',
    summonerLevel: 77,
    tier: 'BRONZE',
    division: 'I',
    lp: 55,
    wins: 22,
    losses: 25,
    mainRole: 'MID',
    champions: [{ id: 99, name: 'Lux', games: 5, wins: 3 }],
    kills: 3.4,
    deaths: 6.1,
    assists: 6.8,
    recentGames: 7,
    verifiedAt: null,
    syncedAt: '2026-08-08T02:35:00.000Z',
  },
]

const GAME_ACCOUNTS = [
  {
    id: 'ga-pubg-potato',
    userId: 'u-potato',
    game: 'pubg',
    platform: 'kakao',
    name: '감자칩',
    externalId: 'seed-pubg-potato',
    tier: '다이아몬드 3',
    detail: '스쿼드 1인칭 · 4,123 RP · 128판',
    stats: { mode: 'squad-fpp', rp: 4123, rounds: 128, wins: 11, kda: 3.42, avgDamage: 412, winRate: 9 },
    syncedAt: '2026-08-08T03:00:00.000Z',
  },
  {
    id: 'ga-pubg-overtime',
    userId: 'u-overtime',
    game: 'pubg',
    platform: 'steam',
    name: 'NightShift',
    externalId: 'seed-pubg-overtime',
    tier: '플래티넘 1',
    detail: '듀오 · 3,480 RP · 64판',
    stats: { mode: 'duo', rp: 3480, rounds: 64, wins: 4, kda: 2.18, avgDamage: 318, winRate: 6 },
    syncedAt: '2026-08-08T04:00:00.000Z',
  },
  {
    id: 'ga-fconline-coach',
    userId: 'u-coach',
    game: 'fconline',
    platform: null,
    name: '코치정',
    externalId: 'seed-fc-coach',
    tier: '월드클래스1',
    detail: '공식경기 최고등급 · Lv.87',
    stats: { level: 87, division: 1000 },
    syncedAt: '2026-08-08T03:30:00.000Z',
  },
  {
    id: 'ga-maple-mapler',
    userId: 'u-mapler',
    game: 'maple',
    platform: null,
    name: '단풍잎',
    externalId: 'seed-maple-mapler',
    tier: 'Lv.262 아델',
    detail: '스카니아 · 단풍길드 · 전투력 1.8억',
    stats: { level: 262, power: 184000000, world: '스카니아', job: '아델' },
    syncedAt: '2026-08-08T02:40:00.000Z',
  },
  {
    id: 'ga-sudden-overtime',
    userId: 'u-overtime',
    game: 'sudden',
    platform: null,
    name: '야근전사',
    externalId: 'seed-sa-overtime',
    tier: '준장 3호봉',
    detail: '92레벨 · 매너 우수',
    stats: { level: 92 },
    syncedAt: '2026-08-08T04:10:00.000Z',
  },
  {
    id: 'ga-wow-nightowl',
    userId: 'u-nightowl',
    game: 'wow',
    platform: 'hyjal',
    name: '밤활',
    externalId: 'hyjal:seed-wow-nightowl',
    tier: '80레벨 사격 사냥꾼',
    detail: '하이잘 · 달빛술사 · 아이템 레벨 636',
    stats: { level: 80, itemLevel: 636, job: '사냥꾼', world: '하이잘' },
    syncedAt: '2026-08-08T05:00:00.000Z',
  },
  {
    id: 'ga-d3-midking',
    userId: 'u-midking',
    game: 'd3',
    platform: null,
    name: '미드장인#3721',
    externalId: '미드장인-3721',
    tier: '파라곤 1,204',
    detail: '악마사냥꾼 · 영웅 6명',
    stats: { paragon: 1204, heroes: 6, job: '악마사냥꾼' },
    syncedAt: '2026-08-08T05:20:00.000Z',
  },
  {
    id: 'ga-ow-overtime',
    userId: 'u-overtime',
    game: 'overwatch',
    platform: null,
    name: '야근전사#3311',
    externalId: 'seed-ow-overtime',
    tier: '다이아몬드 2',
    detail: '탱커 · 승률 54% · 시즌 128판',
    stats: { rounds: 128, winRate: 54, job: '탱커' },
    syncedAt: '2026-08-08T04:20:00.000Z',
  },
  {
    id: 'ga-val-newbie',
    userId: 'u-newbie',
    game: 'valorant',
    platform: null,
    name: '초보탈출#KR3',
    externalId: 'seed-val-newbie',
    tier: '아이언 2',
    detail: '타격대 · 승률 38% · 42판',
    stats: { rounds: 42, winRate: 38, job: '타격대' },
    syncedAt: '2026-08-07T13:00:00.000Z',
  },
  {
    id: 'ga-lostark-mapler',
    userId: 'u-mapler',
    game: 'lostark',
    platform: null,
    name: '단풍잎',
    externalId: 'seed-loa-mapler',
    tier: '아이템 레벨 1,675',
    detail: '카마인 · 브레이커 · 원정대 62',
    stats: { itemLevel: 1675, level: 62, job: '브레이커', world: '카마인' },
    syncedAt: '2026-08-08T02:50:00.000Z',
  },
  {
    id: 'ga-dnf-coach',
    userId: 'u-coach',
    game: 'dnf',
    platform: null,
    name: '코치정',
    externalId: 'seed-dnf-coach',
    tier: '명성 78,400',
    detail: '카인 · 眞 버서커 · Lv.110',
    stats: { level: 110, power: 78400, job: '버서커', world: '카인' },
    syncedAt: '2026-08-08T03:40:00.000Z',
  },
  {
    id: 'ga-sc-midking',
    userId: 'u-midking',
    game: 'starcraft',
    platform: null,
    name: '미드장인',
    externalId: 'seed-sc-midking',
    tier: '마스터',
    detail: '테란 · 시즌 승률 61% · 214판',
    stats: { rounds: 214, winRate: 61, job: '테란' },
    syncedAt: '2026-08-08T05:40:00.000Z',
  },
  {
    id: 'ga-er-nightowl',
    userId: 'u-nightowl',
    game: 'eternalreturn',
    platform: null,
    name: '한밤의궁수',
    externalId: 'seed-er-nightowl',
    tier: '미스릴 4',
    detail: '리 다이린 · 평균 순위 4.2위 · 96판',
    stats: { rounds: 96, kda: 3.1 },
    syncedAt: '2026-08-08T02:20:00.000Z',
  },
  {
    id: 'ga-kart-potato',
    userId: 'u-potato',
    game: 'kart',
    platform: null,
    name: '감자칩',
    externalId: 'seed-kart-potato',
    tier: '프로',
    detail: '스피드전 · 승률 47% · 312판',
    stats: { rounds: 312, winRate: 47 },
    syncedAt: '2026-08-08T03:10:00.000Z',
  },
  {
    id: 'ga-apex-potato',
    userId: 'u-potato',
    game: 'apex',
    platform: null,
    name: 'PotatoChip',
    externalId: 'seed-apex-potato',
    tier: '플래티넘 3',
    detail: '레이스 · 평균 딜 612 · 88판',
    stats: { rounds: 88, avgDamage: 612, kda: 1.9, job: '레이스' },
    syncedAt: '2026-08-08T03:20:00.000Z',
  },
  {
    id: 'ga-lostark-midking',
    userId: 'u-midking',
    game: 'lostark',
    platform: null,
    name: '미드장인',
    externalId: 'seed-loa-midking',
    tier: '아이템 레벨 1,720',
    detail: '아브렐슈드 · 소서리스 · 원정대 68',
    stats: { itemLevel: 1720, level: 68, job: '소서리스', world: '아브렐슈드' },
    syncedAt: '2026-08-08T05:50:00.000Z',
  },
  {
    id: 'ga-val-midking',
    userId: 'u-midking',
    game: 'valorant',
    platform: null,
    name: '미드장인#KR1',
    externalId: 'seed-val-midking',
    tier: '다이아몬드 1',
    detail: '척후대 · 승률 58% · 96판',
    stats: { rounds: 96, winRate: 58, job: '척후대' },
    syncedAt: '2026-08-08T05:55:00.000Z',
  },
  {
    id: 'ga-tl-nightowl',
    userId: 'u-nightowl',
    game: 'tl',
    platform: null,
    name: '한밤의궁수',
    externalId: 'seed-tl-nightowl',
    tier: '55레벨 궁수',
    detail: '카이젠 · 전투력 4,120',
    stats: { level: 55, power: 4120, job: '궁수', world: '카이젠' },
    syncedAt: '2026-08-08T02:15:00.000Z',
  },
  {
    id: 'ga-lineage-coach',
    userId: 'u-coach',
    game: 'lineage',
    platform: null,
    name: '코치정',
    externalId: 'seed-lin-coach',
    tier: '92레벨 군주',
    detail: '기란 · 혈맹 무진 · 무기 +9',
    stats: { level: 92, job: '군주', world: '기란' },
    syncedAt: '2026-08-08T03:50:00.000Z',
  },
  {
    id: 'ga-cs2-potato',
    userId: 'u-potato',
    game: 'cs2',
    platform: null,
    name: 'PotatoChip',
    externalId: 'seed-cs2-potato',
    tier: '프리미어 14,200',
    detail: '승률 52% · 240판',
    stats: { rounds: 240, winRate: 52, kda: 1.14 },
    syncedAt: '2026-08-08T03:25:00.000Z',
  },
  {
    id: 'ga-mabinogi-mapler',
    userId: 'u-mapler',
    game: 'mabinogi',
    platform: null,
    name: '단풍잎',
    externalId: 'seed-mabi-mapler',
    tier: '누적 12,400',
    detail: '데이안 · 음유시인',
    stats: { level: 12400, job: '음유시인', world: '데이안' },
    syncedAt: '2026-08-08T02:45:00.000Z',
  },
  {
    id: 'ga-dnf-newbie',
    userId: 'u-newbie',
    game: 'dnf',
    platform: null,
    name: '초보탈출',
    externalId: 'seed-dnf-newbie',
    tier: '명성 12,800',
    detail: '카시야스 · 眞 소울브링어 · Lv.110',
    stats: { level: 110, power: 12800, job: '소울브링어', world: '카시야스' },
    syncedAt: '2026-08-07T13:30:00.000Z',
  },
  {
    id: 'ga-er-overtime',
    userId: 'u-overtime',
    game: 'eternalreturn',
    platform: null,
    name: '야근전사',
    externalId: 'seed-er-overtime',
    tier: '플래티넘 2',
    detail: '아야 · 평균 순위 6.1위 · 54판',
    stats: { rounds: 54, kda: 2.2 },
    syncedAt: '2026-08-08T04:40:00.000Z',
  },
  {
    id: 'ga-aion-nightowl',
    userId: 'u-nightowl',
    game: 'aion',
    platform: null,
    name: '밤의궁수',
    externalId: 'seed-aion-nightowl',
    tier: '80레벨 궁성',
    detail: '지그하르트 · 어비스 3계급',
    stats: { level: 80, job: '궁성', world: '지그하르트' },
    syncedAt: '2026-08-08T02:25:00.000Z',
  },
]


const LISTINGS = [
  {
    id: 'l-1',
    userId: 'u-midking',
    kind: 'TEACH',
    meetMode: 'BOTH',
    title: '롤 라인전 알려드려요 (미드/탑)',
    body: 'CS 놓치는 이유부터 잡아드립니다. 리플레이 같이 보면서 피드백해요. 온라인은 디스코드, 만나서 하실 분은 강남 쪽 PC방에서 가능합니다.',
    mainGame: 'lol',
    games: ['lol'],
    tier: '마스터',
    myRole: 'MID',
    wantRoles: ['ADC', 'SUPPORT'],
    pricePerHour: 20000,
    region: '서울 강남구',
    pcbang: '레벨업 PC방 강남점',
    availableFrom: '13:00',
    availableTo: '26:00',
    createdAt: '2026-08-04T02:00:00.000Z',
  },
  {
    id: 'l-2',
    userId: 'u-newbie',
    kind: 'LEARN',
    meetMode: 'ONLINE',
    title: '발로란트 진짜 기초부터 알려주세요',
    body: '에임도 없고 맵도 몰라요. 욕 안 하시는 분이면 좋겠습니다. 평일 저녁에 시간 됩니다.',
    mainGame: 'valorant',
    games: ['valorant'],
    tier: '아이언',
    myRole: 'DUELIST',
    wantRoles: ['INITIATOR', 'CONTROLLER'],
    pricePerHour: 10000,
    region: '온라인',
    availableFrom: '19:00',
    availableTo: '23:00',
    createdAt: '2026-08-05T08:30:00.000Z',
  },
  {
    id: 'l-3',
    userId: 'u-nightowl',
    kind: 'PLAY',
    meetMode: 'ONLINE',
    title: '새벽에 롤 같이 하실 분 (즐겜)',
    body: '새벽 1시 이후에 주로 켭니다. 티어 상관없고 그냥 웃으면서 하실 분이면 좋아요. 무료입니다.',
    mainGame: 'lol',
    games: ['lol', 'overwatch'],
    tier: '플래티넘',
    myRole: 'ADC',
    wantRoles: ['SUPPORT'],
    pricePerHour: 0,
    region: '온라인',
    availableFrom: '01:00',
    availableTo: '05:00',
    createdAt: '2026-08-05T15:10:00.000Z',
  },
  {
    id: 'l-4',
    userId: 'u-coach',
    kind: 'TEACH',
    meetMode: 'ONLINE',
    title: 'FC온라인 감독모드·전술 세팅 알려드립니다',
    body: '스쿼드 짜는 법, 전술 수치 조정까지 화면 공유로 알려드려요. 랭크 올리고 싶은 분 환영합니다.',
    mainGame: 'fconline',
    games: ['fconline'],
    tier: '챔피언스',
    pricePerHour: 25000,
    region: '온라인',
    availableFrom: '11:00',
    availableTo: '22:00',
    createdAt: '2026-08-02T05:00:00.000Z',
  },
  {
    id: 'l-5',
    userId: 'u-potato',
    kind: 'PLAY',
    meetMode: 'OFFLINE',
    title: '배그 스쿼드 한 자리 — PC방에서 만나서 해요',
    body: '관악구 근처 PC방에서 주말 오후에 모입니다. 4명 중 1자리 비었어요. 자리값은 각자 부담이고 참가비는 없습니다.',
    mainGame: 'pubg',
    games: ['pubg'],
    tier: '골드',
    pricePerHour: 0,
    region: '서울 관악구',
    pcbang: '레벨업 PC방 신림점',
    availableFrom: '14:00',
    availableTo: '20:00',
    createdAt: '2026-08-06T01:20:00.000Z',
  },
  {
    id: 'l-6',
    userId: 'u-overtime',
    kind: 'PLAY',
    meetMode: 'BOTH',
    title: '퇴근하고 오버워치 한 판씩 같이 해요',
    body: '평일 밤 9시 이후. 경쟁전보다 빠른대전 위주입니다. 광진구 사시면 만나서도 좋고요.',
    mainGame: 'overwatch',
    games: ['overwatch', 'valorant'],
    tier: '다이아',
    myRole: 'TANK',
    wantRoles: ['DAMAGE', 'HEALER'],
    pricePerHour: 0,
    region: '서울 광진구',
    pcbang: '레벨업 PC방 건대점',
    availableFrom: '21:00',
    availableTo: '25:00',
    createdAt: '2026-08-05T11:00:00.000Z',
  },
  {
    id: 'l-7',
    userId: 'u-mapler',
    kind: 'LEARN',
    meetMode: 'ONLINE',
    title: '메이플 복귀했는데 뭐부터 해야 할지 알려주세요',
    body: '5년 만에 돌아왔습니다. 육성 루트랑 요즘 사냥터 알려주실 분 찾아요. 사례는 드립니다.',
    mainGame: 'maple',
    games: ['maple'],
    tier: '복귀',
    pricePerHour: 15000,
    region: '온라인',
    availableFrom: '20:00',
    availableTo: '24:00',
    createdAt: '2026-08-03T12:00:00.000Z',
  },
  {
    id: 'l-8',
    userId: 'u-midking',
    kind: 'PLAY',
    meetMode: 'OFFLINE',
    title: '주말에 강남에서 롤 5인큐 하실 분',
    body: '토요일 오후에 PC방에서 5인 모아서 돌립니다. 지금 2자리 남았어요.',
    mainGame: 'lol',
    games: ['lol'],
    tier: '마스터',
    myRole: 'MID',
    wantRoles: ['ADC', 'SUPPORT'],
    pricePerHour: 0,
    region: '서울 강남구',
    pcbang: '레벨업 PC방 강남점',
    availableFrom: '15:00',
    availableTo: '21:00',
    createdAt: '2026-08-01T07:00:00.000Z',
  },
]

const BOOKINGS = [
  {
    id: 'b-1',
    listingId: 'l-1',
    memberId: 'u-newbie',
    hostId: 'u-midking',
    startAt: '2026-08-08T11:00:00.000Z',
    hours: 2,
    amount: 40000,
    meetMode: 'ONLINE',
    status: 'CONFIRMED',
    checkInCode: 'LM-4821',
    createdAt: '2026-08-05T09:00:00.000Z',
  },
  {
    id: 'b-2',
    listingId: 'l-4',
    memberId: 'u-mapler',
    hostId: 'u-coach',
    startAt: '2026-08-04T10:00:00.000Z',
    hours: 1,
    amount: 25000,
    meetMode: 'ONLINE',
    status: 'COMPLETED',
    checkInCode: 'LM-1170',
    createdAt: '2026-08-03T09:00:00.000Z',
  },
]

const MESSAGES = [
  {
    id: 'm-1',
    bookingId: 'b-1',
    senderId: 'u-newbie',
    body: '안녕하세요! 토요일 8시에 뵙기로 한 거 맞죠?',
    createdAt: '2026-08-05T09:05:00.000Z',
  },
  {
    id: 'm-2',
    bookingId: 'b-1',
    senderId: 'u-midking',
    body: '네 맞습니다. 디스코드 링크는 시작 10분 전에 여기로 보내드릴게요.',
    createdAt: '2026-08-05T09:12:00.000Z',
  },
]

const PLANS = [
  {
    id: 'p-free',
    name: '무료',
    target: 'MEMBER',
    monthly: 0,
    feeRate: 0.15,
    features: ['글 올리기 무제한', '기본 검색', '앱 내 채팅'],
    active: true,
    note: '',
    sortOrder: 1,
  },
  {
    id: 'p-mate-basic',
    name: '메이트 베이직',
    target: 'MATE',
    monthly: 0,
    feeRate: 0.15,
    features: ['수수료 15%', '기본 노출'],
    active: true,
    note: '기본 요금제',
    sortOrder: 2,
  },
  {
    id: 'p-mate-pro',
    name: '메이트 프로',
    target: 'MATE',
    monthly: 9900,
    feeRate: 0.1,
    features: ['수수료 10%', '상단 노출', '프로필 강조', '통계 제공'],
    active: false,
    note: '검토 중',
    sortOrder: 3,
  },
  {
    id: 'p-pcbang',
    name: '제휴 PC방',
    target: 'PCBANG',
    monthly: 49000,
    feeRate: 0,
    features: ['매장 노출', '체크인 연동', '이벤트 등록'],
    active: false,
    note: '요금 미확정',
    sortOrder: 4,
  },
]

async function main() {
  // ── 관리자 ──────────────────────────────────────────────
  const adminCount = await db.adminAccount.count()
  if (adminCount === 0) {
    const password = process.env.ADMIN_PASSWORD || 'levelup'
    await db.adminAccount.create({
      data: {
        id: 'a-owner',
        username: process.env.ADMIN_USERNAME || 'levelup',
        passwordHash: hashPassword(password),
        name: '최고 관리자',
        owner: true,
      },
    })
    console.log('관리자 계정을 만들었습니다.')
    if (!process.env.ADMIN_PASSWORD) {
      console.log('  ⚠ 비밀번호가 기본값입니다. 로그인 후 바로 바꿔주세요.')
    }
  }

  // ── 요금제 ──────────────────────────────────────────────
  for (const plan of PLANS) {
    await db.plan.upsert({ where: { id: plan.id }, update: {}, create: plan })
  }

  // ── 예시 회원·글 ────────────────────────────────────────
  // 이미 회원이 있으면 손대지 않는다. 실제 회원이 들어온 뒤에
  // 예시 데이터가 다시 끼어들면 곤란하다.
  if ((await db.user.count()) > 0) {
    console.log('회원이 이미 있어 예시 데이터는 건너뜁니다.')
    return
  }

  for (const u of USERS) {
    await db.user.create({
      data: {
        ...u,
        phoneDigits: digits(u.phone),
        createdAt: new Date(u.createdAt),
      },
    })
  }
  for (const r of RIOT_ACCOUNTS) {
    await db.riotAccount.create({
      data: {
        ...r,
        verifiedAt: r.verifiedAt ? new Date(r.verifiedAt) : null,
        syncedAt: r.syncedAt ? new Date(r.syncedAt) : null,
      },
    })
  }
  for (const g of GAME_ACCOUNTS) {
    await db.gameAccount.create({
      data: { ...g, syncedAt: g.syncedAt ? new Date(g.syncedAt) : null },
    })
  }
  for (const l of LISTINGS) {
    await db.listing.create({ data: { ...l, createdAt: new Date(l.createdAt) } })
  }
  for (const b of BOOKINGS) {
    await db.booking.create({
      data: { ...b, startAt: new Date(b.startAt), createdAt: new Date(b.createdAt) },
    })
  }
  for (const m of MESSAGES) {
    await db.message.create({ data: { ...m, createdAt: new Date(m.createdAt) } })
  }
  console.log(
    `예시 데이터를 넣었습니다. 회원 ${USERS.length}명, 글 ${LISTINGS.length}개, ` +
      `연결된 게임 계정 ${RIOT_ACCOUNTS.length + GAME_ACCOUNTS.length}개.`,
  )
}

main()
  .catch((err) => {
    console.error(err)
    process.exitCode = 1
  })
  .finally(() => db.$disconnect())
